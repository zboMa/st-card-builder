...
function listGraphElementIds(graph) {
  ...
  if (!data) return [];
  var ids = [];
  (data.nodes || []).forEach(function(d) { if (d && d.id != null) ids.push(String(d.id)); });
  (data.edges || []).forEach(function(d) { if (d && d.id != null) ids.push(String(d.id)); });
  (data.combos || []).forEach(function(d) { if (d && d.id != null) ids.push(String(d.id)); });
  return ids;
}

/** 强制清空选中相关态；默认样式需自带 opacity:1，否则 inactive 透明度会粘住 */
function clearGraphSelectStates(graph) {
  if (!graph || graph.destroyed || typeof graph.setElementState !== 'function') return;
  try {
    var states = {};
    listGraphElementIds(graph).forEach(function(id) { states[id] = []; });
    ...
    if (typeof graph.draw === 'function') {
      try { graph.draw(); } catch (e2) { /* ignore */ }
    }
  } catch (e) {
...
function applyGraphHighlight(graph, centerId, degree, kind) {
  ...
  if (kind === 'edge') {
    ...
    var edge = (data.edges || []).find(function(e) { return e && String(e.id) === String(centerId); });
    if (edge) {
...
function bindSelectHandlers(graph, opts) {
  var highlightDegree = opts && opts.highlightDegree != null ? opts.highlightDegree : 2;
  highlightDegree = Math.max(0, Math.min(6, Math.floor(Number(highlightDegree) || 0)));
  if (graph.__novelOnNodeClick) graph.off(NodeEvent.CLICK, graph.__novelOnNodeClick);
  if (graph.__novelOnEdgeClick) graph.off(EdgeEvent.CLICK, graph.__novelOnEdgeClick);
  if (graph.__novelOnCanvasClick) graph.off(CanvasEvent.CLICK, graph.__novelOnCanvasClick);
  ...
  graph.__novelOnNodeClick = function(evt) {
    ...
    // 再次点击同一节点 → 取消高亮
    if (graph.__novelSelectedId && String(graph.__novelSelectedId) === String(id)) {
...
export function mountOrUpdateGraph(container, graphData, existing, opts) {
  ...
  if (highlightDegree == null || !Number.isFinite(Number(highlightDegree))) highlightDegree = 2;
  highlightDegree = Math.max(0, Math.min(6, Math.floor(Number(highlightDegree))));

  // 旧实例若仍带 click-select，或层级变更，直接销毁重建（否则 inactive 透明度会粘住）
  if (existing && typeof existing.destroy === 'function' && !existing.destroyed) {
    var needRebuild = existing.__novelSelectMode !== 'custom'
  ...
  if (existing && typeof existing.destroy === 'function' && !existing.destroyed) {
    ...
    bindSelectHandlers(existing, Object.assign({}, opts, { highlightDegree: highlightDegree }));
    existing.__novelHighlightDegree = highlightDegree;
  ...
  bindSelectHandlers(graph, Object.assign({}, opts, { highlightDegree: highlightDegree }));
