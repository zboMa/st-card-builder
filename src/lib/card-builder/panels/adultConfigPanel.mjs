...
export function attachAdultConfigPanel(ctx, s, panel) {
  Object.assign(panel, {
    ...
    renderWorldviewPresetList: function() {
        ...
        if (!items.length) {
          ...
          listEl.innerHTML = items.map(function(it, idx) {
            ...
            return '<div class="adult-dashed-item" data-wv-idx="' + idx + '">'
              ...
              + '<div class="adult-dashed-item-title">' + s.escapeHtml(p.label)
              + (idx === 0
      ...
      if (picker) {
        ...
        listWorldviewPresetsByGroup().forEach(function(g) {
          ...
          opts += '<optgroup label="' + s.escapeHtml(g.label) + '">';
          avail.forEach(function(it) {
            opts += '<option value="' + s.escapeHtml(it.id) + '">' + s.escapeHtml(it.label) + '</option>';
          });
          opts += '</optgroup>';
        });
      ...
      if (!items.length) {
        ...
        listEl.innerHTML = items.map(function(it, idx) {
          ...
          return '<div class="adult-dashed-item" data-flavor-idx="' + idx + '">'
            + '<div class="adult-dashed-item-head">'
            + '<div class="adult-dashed-item-meta">'
            + '<div class="adult-dashed-item-title">' + s.escapeHtml(f.label)
      ...
      if (!items.length) {
        ...
        listEl.innerHTML = items.map(function(it, idx) {
          ...
          return '<div class="adult-dashed-item" data-expression-idx="' + idx + '">'
            + '<div class="adult-dashed-item-head">'
            + '<div class="adult-dashed-item-meta">'
            + '<div class="adult-dashed-item-title">' + s.escapeHtml(p.label) + '</div>'
            + (blurb ? '<div class="adult-dashed-item-desc">' + s.escapeHtml(blurb) + '</div>' : '')
            + '</div>'
            + '<div class="adult-dashed-item-actions">'
      ...
      if (picker) {
        ...
        groups.forEach(function(g) {
          ...
          groupIds.forEach(function(id) {
            ...
            opts += '<option value="' + id + '">' + s.escapeHtml((p && p.label) || id) + s.escapeHtml(sm) + '</option>';
          });
          opts += '</optgroup>';
        });
      ...
      if (ntlContainer && data) {
        ...
        ntlContainer.innerHTML = data.tabooIds.map(function(id) {
          ...
          return '<button type="button" class="novel-chip-btn' + (active ? ' active' : '') + '"'
            + ' data-adult-ntl="' + id + '" title="' + s.escapeHtml(tip) + '"'
            + ' aria-pressed="' + (active ? 'true' : 'false') + '">'
            + s.escapeHtml(info.label)
    ...
    renderNtlList: function() {
      ...
      listEl.innerHTML = items.map(function(it, idx) {
        ...
        return '<div class="adult-dashed-item" data-ntl-idx="' + idx + '">'
          + '<div class="adult-dashed-item-head">'
          + '<div class="adult-dashed-item-meta">'
          + '<div class="adult-dashed-item-title">' + s.escapeHtml(info.label)
    ...
    readNtlItemsFromUi: function() {
      ...
      s.ensureNtlItemsOnState().forEach(function(it) { noteMap[it.id] = it.note || ''; });
