...
export const DEFAULT_PROMPTS = {
  ...
  assistantSystem: join(
    ...
    '【对话方式】像现代智能体一样自然对话：默认用简洁中文直接回复用户；'
    + '只有需要读卡、改卡、查世界书、跑分析等操作时才调用工具；不要为了「走流程」硬调工具。\n',
    '规则：\n',
    ...
    '4. 每次最多调用一个工具；信息已够则直接用自然语言回答，不要空转。\n',
    ...
    '\n{{catalogOverview}}\n',
    '\n【输出】\n',
    '- 普通回复：直接输出给用户的中文，不要包 JSON，不要写 thought/final 外壳。\n',
    '- 需要工具时：只输出一个 JSON 对象（可放在 ```json 围栏内），格式：'
    + '{"thought":"一句话理由","tool":"工具名","args":{}}；前后不要夹其它说明。\n',
    '- 兼容：若仍输出 {"final":"..."} 也可以，系统会取出 final 正文展示。\n'
  ),
