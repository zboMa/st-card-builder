...
describe('storyStudio UI mount', function() {
  it('面板与入口 DOM id 存在', function() {
    ...
    assert.match(app, /ss-novel-badge/);
    assert.match(app, /当前/);
    assert.match(app, /writeChapterId/);
    assert.match(app, /collectWriteFromDom\(\{\s*chapterId:\s*prevId\s*\}\)/);
    ...
    assert.match(styles, /\.ss-novel-badge/);
    assert.doesNotMatch(styles, /\.ss-current-novel-select/);
    const graph = readFileSync(join(root, 'src/components/storyStudio/StoryGraphPanel.astro'), 'utf8');
    assert.match(graph, /btn-inline/);
    assert.match(graph, /novel-graph-wrap/);
    assert.match(graph, /ssGraphHighlightDepth/);
    assert.match(graph, /ssGraphPersonOnly/);
    assert.match(graph, /ssGraphLegend/);
    assert.doesNotMatch(graph, /ss-current-novel-select/);
    assert.doesNotMatch(graph, /ssGraphEditFold/);
    assert.doesNotMatch(graph, /ssGraphNodes/);
    assert.doesNotMatch(graph, /btn-fetch(?!-)/);
    const graphUi = readFileSync(join(root, 'src/lib/storyStudio/graphUi.mjs'), 'utf8');
    assert.match(graphUi, /openStoryGraphCtxMenu/);
    assert.match(graphUi, /openEditNodeDialog/);
    assert.match(graphUi, /showSsConfirm/);
    const graphView = readFileSync(join(root, 'src/lib/storyStudio/graphView.mjs'), 'utf8');
    assert.match(graphView, /onNodeContextMenu/);
    assert.match(graphView, /filterKnowledgeGraphByTypes/);
