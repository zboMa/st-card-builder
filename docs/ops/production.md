...
# 浏览器经 /api/data 存取；Couch 仅本机。PUBLIC_COUCH_URL 已不需要。
...
- **浏览器**只访问 `PUBLIC_API_URL` 的 `/api/data/*`（Session），**不**直连 Couch
- 旧 `/api/sync/credentials` 已 410；公网 Nginx `/couch/` 可移除
- Nginx 示例：`deploy/nginx-card-api.conf.example`（至少 `/api/` → `:8787`）
