...
7. **小说创作（story-studio）**：五视图面板全宽铺满主栏；**操作在面板右上角**；空态用居中 tip；阅读正文列可限宽。图谱用 G6（同小说分析）；大纲标题点击编辑；写作主 CTA「写作」统一模式配置 + 流式正文 + 步骤/任务中心进度；同质操作不并排堆按钮；写作/阅读章导航在正文下方共用 `.ss-read-nav`；写作目录左侧滑出；阅读模式用 `.ss-text-select` 文字下拉。
