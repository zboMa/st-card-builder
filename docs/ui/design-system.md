...
3. 底栏仅操作：左图标组（复制 / 删除）、右「更多」⋯，**一行**；更多菜单挂 `document.body` + `position: fixed`，避免卡片 `overflow: hidden` 裁切。
4. 点名称 → 重命名；点封面其余区域 → 打开；不设独立重命名图标。
5. 标签筛选浮层同样可挂 body（高 z-index），避免被卡片网格盖住。
...
7. **小说创作（story-studio）**：五视图面板全宽铺满主栏；工具栏统一 `btn btn-ghost btn-inline`；空态用居中 tip；阅读正文列可限宽。
